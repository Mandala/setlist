...
describe('Async wrapper $.async() test', function() {
    it('Should throw when passed non-function object', function() {
        (function() {
            $.async(s.obj);
        }).should.throwError('InvalidParameter');
    });
    ...
    it('Should resolve the promise with no parameter', function() {
        return $.async(s.callback)(undefined)
            .should.fulfilledWith(undefined);
    });
    it('Should resolve the promise with single parameter', function() {
        return $.async(s.callback)(true)
            .should.fulfilledWith(true);
    });
    it('Should resolve the promise with multiple parameter', function() {
        return $.async(s.callbackMulti)(true, false, null, 1)
            .should.fulfilledWith([true, false, null, 1]);
    });
});
describe('Task worker $.worker() test', function() {
    it('Should return value from GenFn', function() {
        return s.probe($.worker, s.gf(true)).should
            .fulfilledWith(true);
    });
    it('Should return value from child GenFn', function() {
...
describe('Setlist $ test', function() {
    it('Should throw error on non-GenFn parameter input', function() {
        (function() {
            $(s.fn());
        }).should.throwError('InvalidParameter');
    });
